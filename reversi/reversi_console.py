...
class Game:
    ...
    def isFinal(self):
        ...
        # NICIUNUL NU MAI POATE FACE MUTARI, DECI AM AJUNS LA FINAL
    ...
    def calculate_heuristic_2(self, depth):
        ...
        if situation_final == self.JMIN:
            ...
            return 10 * (nr_corners_jmin - nr_corners_jmax) + (nr_moves_jmin - nr_moves_jmax) // (
                        nr_moves_jmin + nr_corners_jmax)
    ...
    while True:
        ...
        if currState.currPlayer == newGame.JMIN: #E RANDUL PLAYER-ULUI
            if check_can_put(currState.table_game.table, currState.currPlayer) == False: #VERIFICA DACA POATE PUNE PIESE
                ...
                print("Continue or Exit? ") #I SE OFERA 2 OPTIUNI: DE A CONTINUA JOCUL SI DE IESI DIN JOC
                get_out = ' '
                while True:
                    get_out = input()
                    if get_out in ['Exit', 'Continue']:
                        break
                    else:
                        print("Wrong input. Retry! ")
                if get_out == 'Exit': #A ALES SA IASA, ASA CA SE CALCULEAZA SCORURILE SI SE AFISEAZA CASTIGATORUL
                    print("PLAYER QUITED")
                    (nr_JMIN, nr_JMAX) = currState.table_game.calculate_score()
                    print("PLAYER HAS " + str(nr_JMIN) + " POINTS")
                    print("COMPUTER HAS " + str(nr_JMAX) + " POINTS")
                    ...
                    print("PLAYER HAD " + str(counter_player) + " MOVES")
                    break
                while True: #A ALES SA CONTINUE SI #ASTEPTAM SA DEA INPUT VALID
                    print('Choose a box you wanna put your piece: ')
                    print('Give us the row (0->7): ')
                    while True: #ASTEPTAM SA DEA INPUT VALID
                        row = input()
                    ...
                    while True: #ASTEPTAM SA DEA INPUT VALID
                        ...
                        if column in ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']:
                            ...
                            print('Wrong input. Retry!')
                    if currState.table_game.table[row][column] == newGame.Empty: #BOX-UL E LIBER
                        moves = getAllMoves(currState.table_game.table, row, column, currState.currPlayer)
                        if 1 in moves: #VERIFICAM DACA BOX-UL ALES E VALID
                            print("Valid move")
                            counter_player += 1
                            ...
                            print("The table after player's turn")
                            print('  ' + 'a ' + 'b ' + 'c ' + 'd ' + 'e ' + 'f ' + 'g ' + 'h ')
                            for it in range(8):
                                print(str(it), end=" ")
                                for it2 in range(8):
            ...
            if check_can_put(currState.table_game.table, currState.currPlayer) == False: #VERIFICA DACA POATE SA PUNA PIESE
                ...
                if whichAlgorithm == 1:
                    ...
                    initial_time_player = int(round(time.time() * 1000))
            print("The table after computer's turn")
            print('  ' + 'a ' + 'b ' + 'c ' + 'd ' + 'e ' + 'f ' + 'g ' + 'h ')
            for it in range(8):
                print(str(it), end=" ")
                for it2 in range(8):
